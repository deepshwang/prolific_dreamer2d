...
def get_latents(particles, vae, rgb_as_latents=False, use_mlp_particle=False, args=None):
    ...
    if use_mlp_particle:
        ...
        if not rgb_as_latents:
            generator = torch.Generator(device=str(args.device))
            generator.manual_seed(args.seed)
            latents = vae.config.scaling_factor * vae.encode(latents).latent_dist.sample(generator=generator)
    else:
        if rgb_as_latents:
            ...
            generator = torch.Generator(device=str(args.device))
            generator.manual_seed(args.seed)
            latents = vae.config.scaling_factor * vae.encode(rgb_BCHW_512).latent_dist.sample(generator=generator)
    return latents
