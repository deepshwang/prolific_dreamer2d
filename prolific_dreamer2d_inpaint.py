...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument('--init_img', type=str2bool, default=True, help='initialize half of particle with image')
    ### sampling
    ...
    args.work_dir = f'inpaint_{args.work_dir}_{args.run_id}_{args.generation_mode}_cfg_{args.guidance_scale}_bs_{args.batch_size}_num_steps_{args.num_steps}_tschedule_{args.t_schedule}_lr_{args.lr}'
...
def main():
    ...
    if args.use_mlp_particle:
        ...
        if args.init_img:
            ...
            print("Generating test image for inpainting...")
            init_image = torch.from_numpy(np.array(pipe(args.prompt).images[0])).unsqueeze(0).permute(0, 3, 1, 2) / 255.
            init_image = init_image * 2 - 1  #[-1,1]
            del pipe

            if args.rgb_as_latents:
                particles_image = vae.config.scaling_factor * vae.encode(init_image.to(device)).latent_dist.sample().detach().to(device, dtype=dtype)
                particles_model = torch.randn((args.batch_size, unet.config.in_channels, args.height // 8, args.width // (8 * 2)))
            else:
                particles_image = init_image[..., :args.width // 2].to(device, dtype=dtype)
                particles_model = torch.randn((args.batch_size, 3, args.height, args.width // 2))
        else:
            if args.rgb_as_latents:
    ...
    particles_model = particles_model.to(device, dtype=dtype)
    if args.use_mlp_particle:
        ...
        particles_model.requires_grad = True
        particles_to_optimize = [particles_model]
    ...
    if args.generation_mode == 't2i':
        ...
        latents = get_latents(particles, vae, args.rgb_as_latents, args=args)
        if args.half_inference:
    ...
    ### sds text to image generation
        ...
        for step, chosen_t in enumerate(pbar):
            # get latent of all particles
            particles = torch.cat((particles_image, particles_model), dim=-1)
            latents = get_latents(particles, vae, args.rgb_as_latents, use_mlp_particle=args.use_mlp_particle, args=args)
            ...
            # loss = 0.5 * F.mse_loss(latents_vsd[..., args.width // 16:], target[..., args.width // 16:], reduction="mean") / args.batch_size
            loss.backward()
            optimizer.step()
            if args.use_scheduler:
            ...
            if args.log_steps and (step % args.log_steps == 0 or step == (args.num_steps-1)):
                ...
                with torch.no_grad():
                    ...
                    if args.rgb_as_latents:
                        ...
                        image_ = torch.cat((particles_image, particles_model), dim=-1)
                    if args.save_x0:
                        if args.half_inference:
                            pred_latents = pred_latents.half()
                        image_x0 = vae.decode(pred_latents / vae.config.scaling_factor).sample.to(torch.float32)
    ...
    if args.generation_mode == 't2i':
        ...
        image = torch.cat((particles_image, particles_model), dim=-1)
