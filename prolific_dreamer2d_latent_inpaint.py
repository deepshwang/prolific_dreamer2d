...
def main():
    ...
    if args.use_mlp_particle:
        ...
        if args.init_img_path:
            ...
            if args.rgb_as_latents:
                ...
                particles = torch.randn((args.batch_size, unet.config.in_channels, args.height // 8, args.width // (8 * 2)))
            else:
    ...
    ### sds text to image generation
        ...
        for step, chosen_t in enumerate(pbar):
            # get latent of all particles
            latents = torch.cat((input_image[..., :args.width // 16], particles), dim=-1)
                ...
                with torch.no_grad():
                    ...
                    if args.save_x0:
                        ...
                        if args.generation_mode == 'vsd':
                            ...
                            image = torch.cat((init_image.detach().cpu(), image_.cpu()), dim=2)
                        else:
                            image = torch.cat((image_,image_x0), dim=2)
                    else:
                        image = image_
