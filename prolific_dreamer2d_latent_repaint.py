...
from rsd_utils import sds_vsd_grad_diffuser_repaint
import shutil
import logging
...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument('--init_img', type=str2bool, default=True, help='init particle from a generated image')
    args = parser.parse_args()
    # create working directory
    args.run_id = args.run_date + '_' + args.run_time
    args.work_dir = f'repaint_{args.work_dir}_{args.run_id}_{args.generation_mode}_cfg_{args.guidance_scale}_bs_{args.batch_size}_num_steps_{args.num_steps}_tschedule_{args.t_schedule}'
...
def main():
    ...
    if args.use_mlp_particle:
        ...
        if args.init_img_path:
            ...
            if args.rgb_as_latents:
                input_image = vae.config.scaling_factor * vae.encode(init_image.to(device)).latent_dist.sample().detach().to(device, dtype=dtype)
                particles = torch.randn((args.batch_size, unet.config.in_channels, args.height // 8, args.width // 8))
                mask = torch.ones_like(input_image)
                mask[..., :args.width // (8 * 2)] = 0 # Filling in right side of image for test
            else:
                raise NotImplementedError # rgb_as_latents must be true
                # particles_image = init_image.to(device, dtype=dtype)
                # particles_model = torch.randn((args.batch_size, unet.config.in_channels, args.height, args.width // 2))
        else:
            if args.rgb_as_latents:
                particles = torch.randn((args.batch_size, unet.config.in_channels, args.height // 8, args.width // 8))
            else:
    ...
    ### sds text to image generation
        ...
        for step, chosen_t in enumerate(pbar):
            ...
            # loss step
            with torch.no_grad():
                grad_, noise_pred, noise_pred_phi = sds_vsd_grad_diffuser_repaint(unet, noise, text_embeddings_vsd, t, \
            ...
            if args.generation_mode == 'vsd':
                ...
                for _ in range(args.phi_update_step):
                    ...
                    loss_phi = phi_vsd_grad_diffuser(unet_phi, latents_phi.detach(), noise_phi, text_embeddings_phi, t_phi, \
            ...
            if args.log_steps and (step % args.log_steps == 0 or step == (args.num_steps-1)):
                ...
                with torch.no_grad():
                    ...
                    if args.rgb_as_latents:
                        ...
                        image_ = particles.clone().detach()
                    image = image_
                if args.log_progress:
                    image_progress.append((image/2+0.5).clamp(0, 1))
                s_image = (image/2 + 0.5).clamp(0, 1)
                save_image(torch.cat([init_image, s_image.cpu()]), f'{args.work_dir}/{image_name}_image_step{step}_t{t.item()}.png')
